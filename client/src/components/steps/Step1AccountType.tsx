    ...
    <div className="p-8">
      ...
      <div className="space-y-8 max-w-md mx-auto">
        ...
        <div className="space-y-4">
          <Label className="font-bold text-lg block text-center">اختر نوع العملية</Label>
          <RadioGroup 
            value={accountMode} 
          ...
          >
            ...
            >
              <div className={`p-3 rounded-full ${accountMode === "new" ? "bg-[#1e60a6]" : "bg-gray-100"}`}>
                <UserPlus className={`h-8 w-8 ${accountMode === "new" ? "text-white" : "text-gray-500"}`} />
              </div>
              <RadioGroupItem value="new" id="new" className="sr-only" />
              <Label htmlFor="new" className="cursor-pointer font-bold text-lg text-center">
                مستخدم جديد
              </Label>
              <span className="text-sm text-gray-500 text-center">إنشاء حساب جديد</span>
            </div>
            <div 
              className={`flex flex-col items-center gap-3 p-6 rounded-lg border-2 cursor-pointer transition-all ${
            ...
            >
              <div className={`p-3 rounded-full ${accountMode === "existing" ? "bg-[#1e60a6]" : "bg-gray-100"}`}>
                <LogIn className={`h-8 w-8 ${accountMode === "existing" ? "text-white" : "text-gray-500"}`} />
              </div>
              <RadioGroupItem value="existing" id="existing" className="sr-only" />
              <Label htmlFor="existing" className="cursor-pointer font-bold text-lg text-center">
                مستخدم حالي
              </Label>
              <span className="text-sm text-gray-500 text-center">تسجيل الدخول</span>
            </div>
          </RadioGroup>
        </div>
        ...
        {accountMode === "new" && (
          <div className="space-y-4 pt-4 border-t border-gray-200">
            ...
            <RadioGroup 
              ...
              className="space-y-3"
            >
              <div className={`flex items-center space-x-reverse space-x-4 p-4 rounded-lg border cursor-pointer transition-colors ${
                accountType === "citizen" ? "border-[#1e60a6] bg-blue-50" : "border-gray-200 bg-white"
              }`}>
                <RadioGroupItem value="citizen" id="citizen" />
                <Label htmlFor="citizen" className="flex-1 cursor-pointer font-bold text-lg">مواطن / مقيم</Label>
              </div>
              <div className={`flex items-center space-x-reverse space-x-4 p-4 rounded-lg border cursor-pointer transition-colors ${
                accountType === "visitor" ? "border-[#1e60a6] bg-blue-50" : "border-gray-200 bg-white"
              }`}>
